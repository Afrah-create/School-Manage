...
export async function assertReportReadiness(
  ...
  if (pending.length > 0 && !options?.allowPartial) {
    ...
    throw new HttpError(
      400,
      `Report cards cannot be generated yet. These subjects still have draft or incomplete marks: ${names}. Ask teachers to submit all assessments first.`,
    );
  }
  ...
  if ((studentCount[0]?.c ?? 0) === 0) {
    throw new HttpError(400, "This class has no active students to include on report cards.");
  }

  return { ...ctx, subjects, pendingSubjects: pending, activeStudents: studentCount[0]!.c };
}
